...
public class CanopyC : ADCComponent
{
    public CanopyC() : base(
        ...
        "通过轮廓曲线来生成林冠线",
        "基础",
        ComponentID.LTCanopyC, 1, Resources.林冠线)
    {
    ...
    {
        #region 初始化 获取输入
        if (!DA.OutData(0, out Curve b, new PolyCurve()) || RMNoValid(b, 0) || RMNoClosed(b, 0) || RNNoPlanar(b, 0, out Plane plane) || //获取线框，并检测闭合与平面
        ...
        var ra = bs.Where(c => c.GetLength() > l)//剔除过短的线
            .Select(c =>
            {
                ...
                return new BrepPopulation(sur, s).Populate(count, null) //获取随机点
                    ...
                    .CreateBooleanUnion(DocumentTolerance()) /// //转成圆组并求交集
