...
public sealed class LTRW : GradientComponent
{
        ...
        ComponentID.LTRW, 3, Resources.实时山路绘制反馈)
    {
        ...
        PreviewExpired += (_, _) => Ct.Clear();
        PreviewExpired += (_, _) => Ma.Clear();
        HL = Math.PI;
    }
    ...
    /// 物件查找设定
    /// </summary>
    private static readonly ObjectEnumeratorSettings Settings = new()
    {
        ActiveObjects = true,
        LockedObjects = true,
        HiddenObjects = true,
...
/// 山路坡度分析_属性
/// </summary>
public sealed class LTRW_Attributes(IGH_Component component) : GH_ComponentAttributes(component)
{
